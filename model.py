...
class VGAE(nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, activation, device, heads=4):
        super(VGAE, self).__init__()
        self.out_channels = out_channels

        self.base_gcn = SAGEConv(in_channels, hidden_channels).to(device)
        self.second_gcn = SAGEConv(hidden_channels, hidden_channels).to(device)

        self.jump_gcn = SAGEConv(in_channels, hidden_channels).to(device)

        self.heads = heads
        self.gcn_mean = GATConv(hidden_channels, out_channels // heads, heads=heads).to(device)
        self.gcn_logstddev = GATConv(hidden_channels, out_channels // heads, heads=heads).to(device)
    ...
    def encode(self, x, edge_index):
        ...
        hidden1 = self.activation(self.base_gcn(x, edge_index))


        hidden2 = self.activation(self.second_gcn(hidden1, edge_index)) + self.jump_gcn(x, edge_index)

        mean_output = self.gcn_mean(hidden2, edge_index)
        logstd_output = self.gcn_logstddev(hidden2, edge_index)

        self.mean = mean_output.view(-1, self.heads * (self.out_channels // self.heads))
        self.logstd = logstd_output.view(-1, self.heads * (self.out_channels // self.heads))

        gaussian_noise = torch.randn_like(self.mean).to(self.device)
...
class Model(torch.nn.Module):
    def __init__(self, encoder: Encoder, vgae: VGAE, tau: float = 0.5):
    ...
    def forward(self, x: torch.Tensor, edge_index: torch.Tensor, edge_index2: torch.Tensor) -> torch.Tensor:
        ...
        z_g = self.vgae.encode(z, edge_index2)
        return z, z_g
...
#fixed
        if len(sub_g1) < 2:
            loss = torch.tensor(0.0, requires_grad=True)  # Ensure this tensor supports gradients
            return loss
    ...
    def vgae_loss(self, A_pred, mean, logstd):
        kl_loss = -(0.5 / A_pred.size(0) )* torch.mean(1 + 2 * logstd - mean.pow(2) - (2 * logstd).exp())
        return kl_loss
